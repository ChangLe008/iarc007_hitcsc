...
//	}
	...
	while (ros::ok())
	{
		g_event.wait_event();
		g_lock.enter();
		//printf("reset running!\n");
		if(record_flag != camera_flag)
		{
			if(camera_flag <= 4){
		    		switch(camera_flag){
...
//		    	select_greyscale_image(CAMERA_ID, true);//标定时需要开启，平时注释掉
//		    	select_greyscale_image(CAMERA_ID, false);//标定时需要开启，平时注释掉

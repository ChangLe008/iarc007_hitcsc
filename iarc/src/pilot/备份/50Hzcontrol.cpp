...
float world_ax=0,world_ay=0,world_az=0,world_vx=0,world_vy=0;
float obs_dx = 0;
float obs_dy = 0;
...
void track(float x,float y,float vx_r,float vy_r,float h_now,float h_ideal,bool att_track,float direction)
{
    ...
    ///////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        if(1)//(period%2==0)
        {
            velocity_arena.vx       = 0;//pid_x.out(0,x,1);
            velocity_arena.vy       = 0;//pid_y.out(0,y,1); 
...
float P01=0.5,P02=0,P03=0,P04=0.1;
float Pre01=0,Pre02=0,Pre03=0,Pre04=0;
float Q01=3,Q02=0,Q03=0,Q04=3;
float dt=0.01;
float Kg_x=0,Kg_v=0;
float R=0.3;
float V_cal=0;
...
float P11=0.5,P12=0,P13=0,P14=0.1;
float Pre11=0,Pre12=0,Pre13=0,Pre14=0;
float Q11=3,Q12=0,Q13=0,Q14=3;
    ...
    */
    ...
    while(ros::ok())
    {
        ...
        if(target_number>=1)
        {
            ...
            //dx=dx*2.6;
            //dy=dy*2.6;
            if(abs(dx-last_dx)>0.1 && last_dx!=0)
                dx=last_dx;
            if(abs(dy-last_dy)>0.1 && last_dy!=0)
                dy=last_dy;
            if(first_time==0)
            {
            ...
            else//param
            {
                ...
                last_Vkf=0;//vx_init;
                Vkf=0;//vx_init;
                P01=0.01;P02=0;P03=0;P04=0.1;
                R=0.3;
                last_Xkf1=dy;
                Xkf1=dy;
                last_Vkf1=0;//vy_init;
                Vkf1=0;//vy_init;
                P11=0.01;P12=0;P13=0;P14=0.1;
                R1=0.3;
            }
        ...
        v_log <<  tic()-time0 <<"\t" << world_vx << "\t" << world_vy << "\t" << wsq_drone->ultrasonic.ranges[1] << "\t" << -1*attitude_command.pitch << "\t" << attitude_command.roll << endl;
        height_log << tic()-time0 <<"\t" << vx << "\t" << vy << "\t" << target_number <<"\t" << dx << "\t" << dy << "\t" << world_ax << "\t" << world_ay << "\t" << world_az << "\t" << Xkf << "\t" << Vkf << "\t" << Xkf1 << "\t" << Vkf1 << "\t" << world_vx << "\t"<< world_vy << "\t" << tar_x << "\t"<< tar_y << "\t" << velocity_arena.vx << "\t" << velocity_arena.vy << endl;
        ...
        switch (control_mode)
        {
            ...
            {              
                ...
                attitude_control = 1; 
               //cout << "test1"<< endl;
                if(1)//(target_number==1)// && landmode==0)
                {  
                    ...
                    {
                        cout << "attitdde"<< endl;
                        track(dx,dy,world_vx,world_vy,wsq_drone->ultrasonic.ranges[1],1.0,1,0.);  // attitude track
                    }//Vkf+0.1,Vkf1+0.1
                    else
                    {
                        cout << "velocity"<< endl;
        ...
        if(1)//(period%2==0)
        {
            ...
            if(attitude_control)
            {
                ...
                attitude_command.pitch  = 0.25+attitude_ground.pitch*cvmGet(R_now_i,0,0)+attitude_ground.roll*cvmGet(R_now_i,0,1);   // -0.7 is the pitch compensate 
                attitude_command.roll   = -0.7+attitude_ground.pitch*cvmGet(R_now_i,1,0)+attitude_ground.roll*cvmGet(R_now_i,1,1);  // -0.7 is the roll compensate 

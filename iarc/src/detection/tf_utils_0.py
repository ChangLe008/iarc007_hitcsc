...
def prediction_run(sess, model, path, num=1200, save_ans=False, pic_mode='jpg'):
	...
	time_all2 = 0
	for i in range(num):		
		datapath = path + str(i+1) + '.' + pic_mode
		print(datapath)
		...
		for region in measure.regionprops(label_image):
			...
			_, contours_g, hierarchy_g = cv2.findContours(green_dilate.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
			_, contours_r, hierarchy_r = cv2.findContours(red_dilate.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
			if len(contours_g) > 0:
				# Draw each contour
				...
				for index_r in range(len(contours_r)):
					if cv2.contourArea(contours_r[index_r]) > 200:
						...
						if (width_r / height_r) < 3.3 and (width_r / height_r) > 0.3:
							box_r = np.int0(cv2.boxPoints(rect_r))

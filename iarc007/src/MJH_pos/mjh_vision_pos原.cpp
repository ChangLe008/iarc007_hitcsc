...
void acc_callback(const dji_sdk::Acceleration& g_acc)
{
	...
	if(location.start){
		...
		if(air_pos[2] > 0.5){
			location.vo_update(2,0.8);
			location.estimate();
			//cout << "accelerometer: >0.3" << endl;
		}
		else{
			location.vo_update(2,0.4);
			location.estimate();
			//cout << "accelerometer: <0.3" << endl;
		}
...
//	fprintf(fp,"\n");
	...
	else
	{
		...
		fprintf(fp2,"x= %f\t y= %f \tz= %f\ttime= %f\n", pilot_pos[0] , pilot_pos[1], pilot_pos[2],t_gui);		
		//air_pos[0] += p_x;
		//air_pos[1] += p_y;
...
*/
	...
	velocity_sub = my_node.subscribe("/guidance/velocity",20, velocity_callback);
	position_sub = my_node.subscribe("/guidance/position",20, position_callback);
	imu_sub = my_node.subscribe("/guidance/imu", 20, imu_callback);
...
*/
			...
			*img = IplImage(img_src);
			...
			/* Extract position of aircraft */
			{
				...
				//将原图像转换为二值图像  
				cv::threshold(src, src, 160, 1, cv::THRESH_BINARY);  
			...
			if(flag)
			{
				...
				for( size_t i = 0; i < lines.size(); i++ )
				{
					...
					if (lines[i][0] >= 0.) {
						...
						if (fabs(lines[i][1] - PI / 2.) < ang) {

							lines[i][1] = PI / 2.;
							lines_flag[i] = 1;
						}
						else if (fabs(lines[i][1] + PI / 2.) < ang) {

							lines[i][1] = -PI / 2.;
							lines_flag[i] = 1;
						}
						else if (fabs(lines[i][1]) < ang) {
						...
						else
						{
                          ...
                          if(lines[i][1] >= 0.){
							if (fabs(lines[i][1] - PI + PI/2.) < ang) {

								lines[i][1] = -PI/2.;
								lines_flag[i] = 1;
							}
							else if (fabs(lines[i][1] - PI + PI) < ang) {
				...
				float tmp_rho_x = 0.4,tmp_rho_y = 0.4;//limit rho
